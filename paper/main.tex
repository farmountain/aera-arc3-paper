...
\paragraph{Notes on this work (author).} \textit{[Placeholder --- the author should replace this paragraph with their actual project history: what triggered the EXPLORE phase idea, which dead-end approaches preceded it, and the FT09 discovery story. The current text in the abstract and experiments was drafted with substantial AI writing assistance over verified experimental data; this paragraph is reserved for first-person provenance that only the human author can supply honestly. See \texttt{paper/ARXIV\_ENDORSEMENT\_REQUEST.md} for context.]}

The FT09 result in \S\ref{sec:experiments} requires its own provenance note: across 4 independent runs, the 0.5B model triggers a logged \texttt{ACTION6} error immediately before \texttt{SOLVED}. The error-then-solve sequence was originally suspected to be a bug; it appears instead to be the win signal for this environment. The result is included with the mechanism stated, not suppressed, because the paper's claim is about exploration efficiency and the mechanism is therefore relevant.
